...
#
# Therefore, we first calculate the sums of both children
# and then calculate the tilt of the current node.
#
#
# The DFS function will return the total sum of the current subtree.
#
# At the same time, we will add every node's tilt to total_tilt.
#
#
# For example:
#
...
# Tilt = |3 - 5| = 2
#
# The subtree sum of node 2 is:
#
# 3 + 2 + 5 = 10
#
# This value 10 is returned to node 4.
#
#
# 🔄 Algorithm
#
# 1. Initialize total_tilt = 0.
#
# 2. Create a DFS function.
#
# 3. If the current node is None:
#       return 0
#
...
class Solution:
    def findTilt(self, root: Optional[TreeNode]) -> int:
        ...
        # DFS function returns the sum of the current subtree
        def dfs(node):

            # If the node is None, its subtree sum is 0
            if node is None:
                return 0
